...
export function CalculatorInput({
    ...
    const formatDisplay = (num: number): string => {
        // Format to 2 decimal places, remove trailing zeros after decimal
    ...
    const handleNumber = (digit: string) => {
        setState(prev => {
            if (prev.hasResult) {
                // After result, start fresh with this digit
                return {
                    displayValue: digit,
                    operator: null,
                    operand: "",
    ...
    const handleDelete = () => {
        setState(prev => {
            ...
            if (!prev.operator) {
                ...
                const newOperand = prev.operand.slice(0, -1);
                return { ...prev, operand: newOperand };
            } else {
                // Remove operator
    ...
    // Show = only when there's a complete expression ready to evaluate
    const hasCompleteExpression = state.operator !== null && state.operand !== "";
    // Show ✓ when: result computed, or value was directly edited (no operator)
