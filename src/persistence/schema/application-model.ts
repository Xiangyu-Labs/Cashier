...
export const sourceDocumentRevisions = sqliteTable(
  ...
  (table) => [
    ...
    index("idx_source_document_revisions_document_created").on(table.sourceDocumentId, table.createdAt),
...
export const storedFiles = sqliteTable(
  ...
  {
    id: text("id").primaryKey().$defaultFn(() => crypto.randomUUID()),
    ledgerId: text("ledger_id").notNull().references(() => ledgers.id, { onDelete: "cascade" }),
...
export const processingOutbox = sqliteTable(
  ...
  (table) => [
    ...
    check("ck_processing_outbox_status", sql`${table.status} IN ('pending', 'claimed', 'completed', 'failed')`),
  ]
);
...
export const uploadSessions = sqliteTable(
  ...
  (table) => [
    ...
    index("idx_upload_sessions_ledger_status_expiry").on(table.ledgerId, table.status, table.expiresAt),
    check("ck_upload_sessions_status", sql`${table.status} IN ('open', 'finalized', 'expired', 'cancelled')`),
  ]
);
...
export const uploadSessionFiles = sqliteTable(
  ...
  (table) => [
    ...
    uniqueIndex("uq_upload_session_files_session_position").on(table.uploadSessionId, table.position),
    ...
    check("ck_upload_session_files_status", sql`${table.status} IN ('planned', 'uploaded', 'finalized', 'rejected')`),
  ]
);
...
export const migrationCheckpoints = sqliteTable(
  ...
  (table) => [
    ...
    check("ck_migration_checkpoints_status", sql`${table.status} IN ('pending', 'running', 'completed', 'failed')`),
  ]
);
