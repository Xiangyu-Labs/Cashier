...
import { getActiveSessionsAction, revokeSessionAction, SessionInfo } from "@/features/auth/server/actions/sessions";
                ...
                >
                    <div className="flex items-center gap-3 overflow-hidden">
                        <div className={`p-2 rounded-full shrink-0 ${session.isCurrent ? 'bg-primary/10 text-primary' : 'bg-[var(--surface)] text-[var(--muted)]'}`}>
                            <DeviceIcon type={session.device.type} os={session.device.os} />
                        </div>
                        <div className="min-w-0">
                            <div className="flex items-center gap-2">
                                <span className="text-sm font-medium text-[var(--text)] truncate">
                                    {session.device.device === "Desktop" || session.device.device === "unknown"
