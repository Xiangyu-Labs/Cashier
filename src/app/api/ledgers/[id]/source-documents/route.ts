...
// PATCH /api/ledgers/[id]/source-documents - 确认来源文档 (将 proposed 转换为 actual ledger entries)
export async function PATCH(request: NextRequest, { params }: RouteParams) {
  try {
    const { id: ledgerId } = await params;
    const body = await request.json();
    const { sourceDocumentId, action } = body;

    if (!sourceDocumentId || action !== 'confirm') {
      return NextResponse.json({ error: "Invalid request" }, { status: 400 });
    }

    const sourceDocument = await db.query.sourceDocuments.findFirst({
      where: and(
    ...
    if (sourceDocument.status !== 'to_confirm') {
      return NextResponse.json({ error: "Source document is not pending confirmation" }, { status: 400 });
    }

    // Convert proposed ledger entries to actual ledger entries
    type ProposedLedgerEntry = {
      amount: number;
    ...
    if (proposed && proposed.length > 0) {
      ...
      for (const entry of proposed) {
        const categoryId = entry.category
          ? allCategories.find(c => c.name === entry.category)?.id ?? null
          : null;
    ...
    logger.error({ error }, "Failed to confirm source document");
    return NextResponse.json(
      { error: "Failed to confirm source document" },
      { status: 500 }
    );
  }
}
