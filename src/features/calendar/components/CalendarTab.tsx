  ...
  const handleNavigate = useCallback(
    (direction: "prev" | "next") => {
      setAnchorDate((current) => {
        switch (viewType) {
          case "month":
            return direction === "prev" ? getPreviousMonth(current) : getNextMonth(current);
          case "year":
            return direction === "prev" ? getPreviousYear(current) : getNextYear(current);
        }
      });
    },
    [viewType]
    ...
    <div className={cn("flex flex-col gap-4", className)}>
      ...
      <div className="flex-1">
        {isLoading ? (
          ...
          <>
            {viewType === "month" && (
              <MonthView anchorDate={anchorDate} data={calendarData} onDayClick={handleDayClick} />
            )}
            {viewType === "year" && (
              <YearView
                anchorDate={anchorDate}
