...
export async function sendEmailChangeCodeAction(inputEmail: string, locale?: string) {
  ...
  const [current, duplicate, existingChallenge] = await Promise.all([
    ...
    db.query.users.findFirst({
      where: and(eq(users.email, newEmail), ne(users.id, userId), isNull(users.deletedAt)),
    }),
  ...
  await db
    ...
    .values({ userId, newEmail, tokenHash, expiresAt })
    .onConflictDoUpdate({
  ...
  if (delivery !== "sent") {
    await db
      ...
      .where(
        and(eq(emailChangeChallenges.userId, userId), eq(emailChangeChallenges.newEmail, newEmail))
      );
    throw new ValidationError("Email delivery is not configured");
  }
...
export async function verifyEmailChangeCodeAction(inputEmail: string, otp: string) {
  ...
  const outcome = await db.transaction(async (tx) => {
    await tx.execute(
      sql`select id from email_change_challenges where user_id = ${userId} for update`
    );
    const challenge = await tx.query.emailChangeChallenges.findFirst({
      where: and(
        eq(emailChangeChallenges.userId, userId),
        eq(emailChangeChallenges.newEmail, newEmail)
      ),
    });
    ...
    if (challenge.lockedUntil != null && challenge.lockedUntil > new Date())
      throw new RateLimitError("Verification is locked");
    ...
    if (!verifyOTP(otp, challenge.tokenHash)) {
      ...
      await tx
        ...
        .set({ attempts, lockedUntil, lastAttemptAt: new Date() })
    ...
    await tx
      .update(users)
      .set({ email: newEmail, emailVerified: new Date(), updatedAt: new Date() })
