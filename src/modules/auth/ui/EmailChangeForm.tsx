...
export function EmailChangeForm({
  ...
  return (
    <div className="w-full space-y-2" data-tab-swipe-ignore>
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
        <div className="flex min-h-11 min-w-0 items-center gap-2 text-sm text-muted-foreground sm:flex-1">
        ...
        <input
          ...
          disabled={pending}
          className="min-h-11 min-w-0 flex-1 rounded-md border border-border bg-bg px-3 text-sm"
        />
        <Button
          className="shrink-0"
          variant="outline"
      ...
      {sent && (
        <div className="flex gap-2">
          ...
          <Button disabled={pending || code.length !== 6} onClick={verify}>
            {pending ? <Loader2 className="h-4 w-4 animate-spin" /> : t("verifyEmail")}
          </Button>
        </div>
      )}
    </div>
