...
export interface OfflineLedgerSnapshotV5 {
  key: string;
  schemaVersion: 5;
  userId: string;
  ledgerId: string;
...
export type OfflineLedgerSnapshot = OfflineLedgerSnapshotV5;

type LegacyOfflineLedgerSnapshot = Omit<
  OfflineLedgerSnapshotV5,
  ...
  databasePromise ??= new Promise((resolve, reject) => {
    ...
    request.onupgradeneeded = () => {
      ...
      if (!db.objectStoreNames.contains(SNAPSHOT_STORE)) {
        ...
        if (!snapshots.indexNames.contains("userId")) {
          snapshots.createIndex("userId", "userId", { unique: false });
        }
      }
      if (!db.objectStoreNames.contains(IMAGE_STORE)) {
        ...
        images.createIndex("snapshotPriority", ["snapshotKey", "viewed", "priorityAt"], {
          unique: false,
        });
        images.createIndex("snapshotAccess", ["snapshotKey", "viewed", "lastAccessedAt"], {
          unique: false,
        });
      } else {
        const images = request.transaction!.objectStore(IMAGE_STORE);
        if (!images.indexNames.contains("snapshotPriority")) {
          images.createIndex("snapshotPriority", ["snapshotKey", "viewed", "priorityAt"], {
            unique: false,
          });
        }
        if (!images.indexNames.contains("snapshotAccess")) {
          images.createIndex("snapshotAccess", ["snapshotKey", "viewed", "lastAccessedAt"], {
            unique: false,
          });
        }
...
export async function readOfflineSnapshot(key: string): Promise<OfflineLedgerSnapshot | null> {
  ...
  if (stored.schemaVersion !== 5) {
    const migrated = migrateOfflineSnapshot(stored);
    const migration = db.transaction(SNAPSHOT_STORE, "readwrite");
    migration.objectStore(SNAPSHOT_STORE).put(migrated);
...
export function migrateOfflineSnapshot(
  ...
  if (snapshot.schemaVersion === 5) return snapshot;
  if (snapshot.schemaVersion === 4) {
    return { ...snapshot, schemaVersion: 5 } as OfflineLedgerSnapshotV5;
  }
  return {
    key: snapshot.key,
    schemaVersion: 5,
...
export async function replaceOfflineSnapshot(snapshot: OfflineLedgerSnapshotV5): Promise<void> {
...
export async function clearOfflineData(userId?: string): Promise<void> {
  ...
  if (userId == null) {
    ...
    const userSnapshots = await requestResult(
      snapshots.index("userId").getAll(userId) as IDBRequest<OfflineLedgerSnapshot[]>
    );
    const keys = new Set(userSnapshots.map((item) => item.key));
...
export async function cacheOfflineImage(input: {
  ...
  const existingStore = existingTx.objectStore(IMAGE_STORE);
  const existing = await requestResult(
    existingStore.get(key) as IDBRequest<OfflineImageRecord | undefined>
  );
  if (existing != null) {
    if (input.viewed) {
      existingStore.put({ ...existing, viewed: true, lastAccessedAt: Date.now() });
  ...
  const remainingBytes = records
    .filter((record) => record.key !== key && !evictionKeys.has(record.key))
    .reduce((total, record) => total + record.byteSize, 0);
  const remainingCount = records.filter(
    (record) => record.key !== key && !evictionKeys.has(record.key)
  ).length;
  if (
...
type OfflineImageCacheInput = Parameters<typeof cacheOfflineImage>[0];

/** Prefetches unviewed images with one IndexedDB scan and one write transaction. */
export async function cacheOfflineImages(
  inputs: readonly OfflineImageCacheInput[],
  signal?: AbortSignal
): Promise<number> {
  const valid = inputs.filter(
    (input) => !input.viewed && input.file.byteSize <= OFFLINE_IMAGE_BYTES_LIMIT
  );
  if (valid.length === 0) return 0;
  const snapshotKey = valid[0]!.snapshotKey;
  if (valid.some((input) => input.snapshotKey !== snapshotKey)) {
    throw new Error("Offline image batch must belong to one snapshot");
  }

  const db = await openOfflineDb();
  const readTx = db.transaction(IMAGE_STORE, "readonly");
  const existing = await requestResult(
    readTx.objectStore(IMAGE_STORE).index("snapshotKey").getAll(snapshotKey) as IDBRequest<
      OfflineImageRecord[]
    >
  );
  const existingKeys = new Set(existing.map((record) => record.key));
  const pending = valid.filter((input) => !existingKeys.has(imageKey(snapshotKey, input.file.id)));
  const downloaded: OfflineImageRecord[] = [];
  let next = 0;
  const worker = async () => {
    while (next < pending.length && signal?.aborted !== true) {
      ...
      const blob = await response.blob();
      if (blob.size <= 0 || blob.size > OFFLINE_IMAGE_BYTES_LIMIT) continue;
      const now = Date.now();
      downloaded.push({
        key: imageKey(snapshotKey, input.file.id),
        snapshotKey,
        fileId: input.file.id,
        documentId: input.documentId,
        contentType: blob.type || input.file.contentType,
        ...
        blob,
        viewed: false,
        priorityAt: Date.parse(input.documentTimestamp) || now,
        lastAccessedAt: now,
      });
    }
  };
  await Promise.all(Array.from({ length: Math.min(3, pending.length) }, worker));
  if (downloaded.length === 0 || signal?.aborted === true) return 0;

  const writeTx = db.transaction(IMAGE_STORE, "readwrite");
  const store = writeTx.objectStore(IMAGE_STORE);
  const current = await requestResult(
    store.index("snapshotKey").getAll(snapshotKey) as IDBRequest<OfflineImageRecord[]>
  );
  const byKey = new Map(current.map((record) => [record.key, record]));
  for (const record of downloaded) if (!byKey.has(record.key)) byKey.set(record.key, record);

  const viewed = [...byKey.values()].filter((record) => record.viewed);
  const unviewed = [...byKey.values()]
    .filter((record) => !record.viewed)
    .toSorted(
      (left, right) => right.priorityAt - left.priorityAt || right.key.localeCompare(left.key)
    );
  const retained = new Set(viewed.map((record) => record.key));
  let bytes = viewed.reduce((total, record) => total + record.byteSize, 0);
  let count = viewed.length;
  for (const record of unviewed) {
    if (
      count + 1 > OFFLINE_IMAGE_COUNT_LIMIT ||
      bytes + record.byteSize > OFFLINE_IMAGE_BYTES_LIMIT
    ) {
      continue;
    }
  ...
  for (const record of current) if (!retained.has(record.key)) store.delete(record.key);
  let inserted = 0;
  for (const record of downloaded) {
    if (retained.has(record.key) && !existingKeys.has(record.key)) {
      store.put(record);
      inserted += 1;
    }
  }
