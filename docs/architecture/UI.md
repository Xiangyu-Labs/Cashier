# Warm Ledger UI Design System

## 1. 设计目标（Design Goals）

这是一套为 Cashier 单账本个人记账产品设计的温润、克制、长期使用型 UI 系统，目标是：

- **账本感**：通过日期分组、金额对齐、清楚的账页结构表达产品性格，而不是纸纹或复古装饰。
- **低认知负担**：核心入口始终是 Stream、Details、Stats、Settings 四个工作区。
- **高一致性**：登录、弹窗、任务队列、设置和四个 tab 共用同一套 token、半径、边框和动效。
- **简单温和**：避免 ChatGPT-like 的聊天工具气质，也避免厚重后台或营销卡片风格。
- **长期维护**：颜色、图表、热力图和状态都使用语义 token，不在组件里散落硬编码颜色。
...
- 主操作和焦点使用 `--primary`
- 收入、支出、警告、危险、信息使用独立语义 token
- 图表和热力图只能引用 `--chart-*` 与 `--heatmap-*`
- 组件中不直接写业务颜色 hex
