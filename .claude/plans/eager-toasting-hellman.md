...
这次重命名不是简单的 find-replace，而是重新定义了领域语言。说明你对"概念准确性"有很高要求——宁可花大力气重命名，也不愿意用不精确的术语。

### Phase 3: 任务引擎三次重写（Batch 10→17→42）
1. **GPT Task System**（Batch 10）：自建简单任务管理 + 数据库状态追踪
2. **Flow + BullMQ**（Batch 17, 36c3649）：引入 Redis + BullMQ 做任务队列，34 文件 1075+/803-
3. **In-process TaskRunner**（Batch 42, c19893f）：移除 Redis/BullMQ，改为进程内 Promise 执行

最终选择：**最简单的方案**。Redis/BullMQ 对单用户记账应用来说是过度工程化。

### Phase 4: 实时更新策略三次迭代（Batch 19→39→51）
1. **SSE + EventBus**（Batch 19, 66f993e）：服务端推送事件，客户端通过 EventSource 监听
2. **Smart Polling + Push Notifications**（Batch 39, a144b85）：轮询替代 SSE
...
- 同时移除了 push notifications（依赖 PostgreSQL 的 JSONB 特性）
...
- 删除了 `src/lib/api.ts` 客户端 API helper
- 删除了 `ApiError` 类
...
最终选择：`[locale]` 段 + `localePrefix='always'`。说明你尝试了"隐藏 locale"的方案但发现问题更多。
...
说明你的舒适区是 **单文件 300-400 行**，超过就需要拆分。
...
| **Tenant isolation via ledgerId** | `forLedger()` helper + `requireLedgerAccess()` | ★★★★★ |
...
### 6.1 "先加后减"模式
你的开发风格是先快速实现功能，然后在使用中发现问题后果断移除。15 个被移除的功能说明你不怕删代码，这是好的工程习惯。

### 6.2 "简化基础设施"倾向
PostgreSQL → SQLite, Redis+BullMQ → 内存 store, SSE → 轮询。对于个人/小团队项目，你倾向于减少外部依赖。

### 6.3 "概念精确"驱动
多次大规模重命名（Transaction→LedgerEntry, Receipt→SourceDocument），说明你认为代码中的命名应该精确反映业务概念。

### 6.4 "标准化模式"追求
当发现重复模式时（乐观更新、mutation 生命周期），你会投入时间创建统一的 factory/hook，而不是容忍 copy-paste。
...
8. **Query Keys**：集中式工厂 + ledger-scoped invalidation predicate
...
### 输出文件 1: `docs/dev-preferences.md`（完整版）
将以上全部内容写入独立文档，作为跨项目可复用的技术偏好参考。

### 输出文件 2: 更新 `CLAUDE.md`（精简版）
在现有 CLAUDE.md 的 `### Development Preferences` 部分追加关键偏好摘要，包括：
- 架构原则（最小复杂度、概念准确性、Server Actions > API Routes）
- 前端核心模式（TanStack Query 标准化、乐观更新 factory、组件行数上限）
- 后端核心模式（throw errors、soft delete、日期字符串、metadata JSONB）
- 决策模式（先加后减、简化基础设施、渐进式重构）

### 验证
- 确认 `docs/dev-preferences.md` 内容完整且格式正确
- 确认 CLAUDE.md 更新不破坏现有结构
