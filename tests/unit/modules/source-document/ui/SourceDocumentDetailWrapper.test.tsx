...
vi.mock("@/modules/source-document/hooks/useSourceDocumentDetailData", () => ({
  useSourceDocumentDetailData: () => ({
    sourceDocument: { id: "doc-1", version: 7, status: documentState.status },
...
describe("SourceDocumentDetailWrapper review navigation", () => {
  ...
  it("retains the candidate review during exit and forwards back", () => {
    documentState.status = "candidate_pending";
    ...
    expect(screen.getByTestId("candidate")).toHaveAttribute("data-open", "true");
    fireEvent.click(screen.getByRole("button", { name: "back" }));
    expect(props.onBack).toHaveBeenCalledTimes(1);
    view.rerender(<SourceDocumentDetailWrapper {...props} open={false} />);
    expect(screen.getByTestId("candidate")).toHaveAttribute("data-open", "false");
