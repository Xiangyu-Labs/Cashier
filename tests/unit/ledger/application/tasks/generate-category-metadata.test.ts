...
describe("generateCategoryMetadataHandler", () => {
  ...
  describe("execute", () => {
    it("should parse valid JSON response and return icon/description", async () => {
      ...
      const result = (await generateCategoryMetadataHandler.execute(
        input,
        context
      )) as GenerateCategoryMetadataOutput;
    ...
    it("should throw on JSON parsing errors (triggers onError)", async () => {
      ...
      await expect(generateCategoryMetadataHandler.execute(input, context)).rejects.toThrow();
    });
  });
