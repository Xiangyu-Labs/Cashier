...
describe("Currency Actions", () => {
    ...
    describe("convertCurrencyAction", () => {
        ...
        it("returns error for missing amount", async () => {
            await expect(convertCurrencyAction(0, "CNY", "USD")).rejects.toThrow("Missing required parameters");
        });

        it("returns error for missing fromCurrency", async () => {
            await expect(convertCurrencyAction(100, "", "USD")).rejects.toThrow("Missing required parameters");
        });

        it("returns error for missing toCurrency", async () => {
            await expect(convertCurrencyAction(100, "CNY", "")).rejects.toThrow("Missing required parameters");
        });
    });
    ...
    describe("batchConvertCurrencyAction", () => {
        ...
        it("returns error for empty items array", async () => {
            await expect(batchConvertCurrencyAction([], "CNY")).rejects.toThrow("Missing required parameters");
        });

        it("returns error for missing target currency", async () => {
            const items = [{ amount: 100, currency: "CNY" }];
            await expect(batchConvertCurrencyAction(items, "")).rejects.toThrow("Missing required parameters");
        });
