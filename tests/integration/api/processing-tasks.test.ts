...
describe("Processing Tasks Action Security", () => {
  ...
  it("should throw error when user is not authenticated", async () => {
    ...
    await expect(getProcessingTasksAction(testLedgerId, {})).rejects.toThrow("Unauthorized");
  });
  ...
  it("should throw error when user requests another user's ledger (IDOR)", async () => {
    ...
    await expect(getProcessingTasksAction(otherLedgerId, {})).rejects.toThrow("Ledger not found");
  });
});
